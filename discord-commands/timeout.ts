...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addNumberOption((option) =>
      option
        .setName("time")
        .setDescription("Time for timeout (minutes)")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("reason")
        .setDescription("Reason to timeout user")
        .setRequired(false)
    ),
  async execute(interaction: any) {
    ...
    const timouted = new MessageEmbed()
      .setColor("#000000")
      .setDescription(
        `You just timeout <@${member.user.id}> for ${times} minutes with ${reason} reason!`
      )
      .setTimestamp()
    ...
    const nopermission = new MessageEmbed()
      .setColor("#000000")
      .setDescription(
        `You don't have permission to timeout user! <:narzelAmogus:961809766244564992>`
      )
      .setTimestamp()
      .setFooter({ text: "Diswitch" });
